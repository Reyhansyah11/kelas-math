...
function testimonialSlider() {
    ...
    if (carouselExampleAutoplaying) {
        carouselExampleAutoplaying.addEventListener('slide.bs.carousel', function (event) {
            const activeItem = event.relatedTarget; // Get the active item
            console.log("Active item:", activeItem); // Log active item for debugging

            const testimonialImage = document.querySelector(".js-testimonial-img");
            const newImageSrc = activeItem.getAttribute("data-js-testimonial-img");

            if (testimonialImage) {
                testimonialImage.src = newImageSrc;
            } else {
                console.error("Testimonial image element not found");
            }
        });
    } else {
...
// 
const colItem = document.querySelectorAll('.col');

colItem.forEach((img, i) => {
    img.dataset.aos = 'fade-down';
    img.dataset.aosDelay = i * 100;
    img.dataset.aosDuration = 1000;
});
